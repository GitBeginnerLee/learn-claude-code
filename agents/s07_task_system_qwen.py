...
class TaskManager:
    ...
    def _max_id(self) -> int:   #统计task目录下的.json文件数量
        ids = [int(f.stem.split("_")[1]) for f in self.dir.glob("task_*.json")]
        return max(ids) if ids else 0

    def _load(self, task_id: int) -> dict:     #读取对应的task.json文件内容
        path = self.dir / f"task_{task_id}.json"
        if not path.exists():
            raise ValueError(f"Task {task_id} not found")
        return json.loads(path.read_text())

    def _save(self, task: dict):    #将task写入到json文件中
        path = self.dir / f"task_{task['id']}.json"
        path.write_text(json.dumps(task, indent=2, ensure_ascii=False)) #覆盖写入

    def create(self, subject: str, description: str = "") -> str:   #创建task，将task数据结构合规化
        task = {
